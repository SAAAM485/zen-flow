import { PrismaClient, FollowRequestStatus, ReactionType } from '@prisma/client';
...
async function main() {
  ...
  const postLikes = [];
  for (const post of posts) {
    const likedUsers = faker.helpers.arrayElements(users, { min: 0, max: users.length / 2 });
  ...
  for (let i = 0; i < users.length * 2; i++) {
    ...
    const status = faker.helpers.arrayElement(Object.values(FollowRequestStatus));
