...
const CommentSection: React.FC<CommentSectionProps> = ({
    ...
    const handleToggleHighlight = async (comment: CommentWithAuthor) => {
        ...
        const actionText = comment.isHighlighted
            ? "Unhighlighted"
            : "Highlighted";
        try {
            ...
            if (!res.ok) {
                ...
                console.error(
                    `Server error during ${actionText.toLowerCase()} comment:`,
                    errorData
                );
                throw new Error(
                    `Failed to ${actionText.toLowerCase()} comment: ${
    ...
    return (
        <div className="mt-4 pt-4 border-t border-border-line">
            ...
            {post.comments.map((comment) => (
                <div
                    key={comment.id}
                    className={`relative ml-4 p-3 rounded-lg transition-colors duration-300 ${
                ...
                >
                    {comment.isHighlighted && (
                        <MapPinIcon className="h-5 w-5 text-yellow-500 absolute top-2 right-2" />
                    )}
                        ...
                        <div className="flex-1">
                            {editingCommentId === comment.id ? (
                                ...
                                <>
                                    ...
                                    <p className="text-sm text-primary-text mt-1">{comment.text}</p>
                                </>
                            )}
            ...
            <form onSubmit={handleCommentSubmit} className="mt-2 ml-4">
                <input
                    ...
                    className="w-full p-2 border border-border-line rounded-md"
                    readOnly={!session}
                />
                <div className="text-right mt-1">
                    <button
                        ...
                        className="text-sm bg-secondary-text text-secondary-bg py-1 px-3 rounded-md hover:bg-primary-text disabled:bg-border-line"
                    >
