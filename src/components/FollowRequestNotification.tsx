...
export default function FollowRequestNotification() {
  ...
  const [pendingRequests, setPendingRequests] = useState<FollowRequestWithFromUser[]>([]);
  ...
  const handleAction = async (requestId: number, action: 'ACCEPTED' | 'REJECTED') => {
    try {
      ...
      console.error(`Error ${action.toLowerCase()} follow request:`, error);
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-primary-text" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 bg-white rounded-md shadow-lg py-1 z-20">
          <div className="block px-4 py-2 text-xs text-gray-400">Follow Requests</div>
          {pendingRequests.length === 0 ? (
            <div className="px-4 py-2 text-sm text-gray-500">No new requests.</div>
          ) : (
            pendingRequests.map(request => (
              <div key={request.id} className="flex items-center justify-between px-4 py-2 hover:bg-gray-100">
                <Link href={`/profile/${request.from.id}`} className="flex items-center flex-grow">
                  <Image
                  ...
                  />
                  <span className="font-semibold text-primary-text">{request.from.name}</span>
                </Link>
