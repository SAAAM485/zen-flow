...
export default function SinglePostPage({ params }: { params: { postId: string } }) {
  ...
  const handleCommentCreated = (newPostId: number, newComment: CommentWithAuthor) => {
    if (post && newPostId === post.id) {
      const commentWithLikes = { ...newComment, commentLikes: [] };
      setPost(prevPost => prevPost ? { ...prevPost, comments: [...prevPost.comments, commentWithLikes] } : null);
    }
  };

  const handleCommentUpdated = (updatedPostId: number, updatedComment: CommentWithAuthor) => {
    if (post && updatedPostId === post.id) {
      setPost(prevPost => prevPost ? {
        ...prevPost,
        comments: prevPost.comments.map(c => c.id === updatedComment.id ? updatedComment : c)
      } : null);
    }
  };
  ...
  const handleSaveEdit = async (id: number) => {
    ...
    try {
      ...
      setPost(prevPost => prevPost ? { ...prevPost, text: updatedPost.text } : null);
